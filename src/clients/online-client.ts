...
export class DcaOnlineClient extends DcaClient {
  ...
  async depositToken(owner: PublicKey, mint: PublicKey, amount: BigNumber) {
    try {
      const { transaction, dcaAccount } =
        await this.makeDepositTokenTransaction(owner, mint, amount);
  ...
  async depositSol(owner: PublicKey, mint: PublicKey, amount: BigNumber) {
    try {
      const { transaction, dcaAccount } = await this.makeDepositSolTransaction(
        owner,
        mint,
        amount
      );
  ...
  async withdrawToken(
    ...
    try {
      const transaction = await this.makeWithdrawTokenTransaction(
        owner,
        mint,
        dcaAccount,
        amount
  ...
  async withdrawSol(
    ...
    try {
      const transaction = await this.makeWithdrawSolTransaction(
        owner,
        mint,
        dcaAccount,
        amount
  ...
  async swapFromSol(owner: PublicKey, mint: PublicKey, dcaAccount: PublicKey) {
    try {
      const transaction = await this.makeSwapFromSolTransaction(
        owner,
        mint,
        dcaAccount
      );
  ...
  async swapToSol(owner: PublicKey, mint: PublicKey, dcaAccount: PublicKey) {
    const transaction = await this.makeSwapToSolTransaction(
      owner,
      mint,
      dcaAccount
    );
  ...
  async fundToken(
    ...
    try {
      const transaction = await this.makeFundTokenTransaction(
        owner,
        mint,
        dcaAccount,
        amount
  ...
  async fundSol(
    ...
    try {
      const transaction = await this.makeFundSolTransaction(
        owner,
        mint,
        dcaAccount,
        amount
