...
export abstract class DcaClient {
  ...
  protected async makeDepositTokenTransaction(
    owner: PublicKey,
    mint: PublicKey,
    amount: BigNumber
  ) {
    try {
      ...
      const vaultNativeMintAddress = await findAssociatedTokenAddress(
        vault,
        NATIVE_MINT
      );
  ...
  protected async makeDepositSolTransaction(
    owner: PublicKey,
    mint: PublicKey,
    amount: BigNumber
  ) {
  ...
  protected async makeWithdrawTokenTransaction(
    ...
    try {
      ...
      const ownerTokenAccount = await findAssociatedTokenAddress(owner, mint);
      const vaultTokenAddress = await findAssociatedTokenAddress(
        vaultAddress,
        mint
      );
      const mintInfo = await getMintInfo(this._connection, mint);
  ...
  protected async makeWithdrawSolTransaction(
    ...
    try {
      const vaultAddress = await findVaultAddress(owner, dcaAccount);
      const ownerTokenAccount = await findAssociatedTokenAddress(owner, mint);
      const ownerNativeMintAccount = await findAssociatedTokenAddress(
        owner,
        NATIVE_MINT
      );
      const vaultTokenAddress = await findAssociatedTokenAddress(
  ...
  protected async makeSwapFromSolTransaction(
    owner: PublicKey,
    mint: PublicKey,
    dcaAccount: PublicKey
  ) {
    try {
      const vault = await findVaultAddress(owner, dcaAccount);
      const vaultNativeMintAccount = await findAssociatedTokenAddress(
        vault,
        NATIVE_MINT
      );
      const vaultTokenAccount = await findAssociatedTokenAddress(vault, mint);
      ...
      const amount = new BigNumber(dcaInfo.dcaAmount.toString()).div(
  ...
  protected async makeSwapToSolTransaction(
    owner: PublicKey,
    mint: PublicKey,
    dcaAccount: PublicKey
  ) {
    ...
    const mintInfo = await getMintInfo(this._connection, mint);
    const amount = new BigNumber(dcaInfo.dcaAmount.toString()).div(
      new BigNumber(10 ** mintInfo.decimals)
    );
    const amountIn = new TokenAmount(
      new Token(poolKeys.baseMint, poolInfo.baseDecimals),
      amount.toString(),
  ...
  protected async makeFundSolTransaction(
    owner: PublicKey,
    mint: PublicKey,
    dcaAccount: PublicKey,
    amount: BigNumber
